  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        title: 'Names we use',
        type: 'p',
        text: 'UI copy says Trial, Pro, Pro Plus, Ultra, Team, Team Premium. Database and checkout still use hobby, pro, pro_plus, ultra, team, team_plus. If you see Hobby in an old screenshot, that is Trial. For the full meter — Studio + Desktop together, images, profit, and competitors — read Pricing & usage.',
      },
    ],
  },
  {
    ...
    toc: [
      ...
      { id: 'profit', title: 'How Soumtok earns' },
    ...
    blocks: [
      {
        type: 'p',
        text: 'Soumtok meters like Cursor: one account, one included pool, every model turn counted by tokens — whether you work in Studio (browser) or Soumtok Desktop (Electron). Dashboard → Usage is the source of truth. Desktop Settings shows the same plan, quota, and top models pulled from your account.',
      },
      {
        id: 'unified',
        ...
        type: 'ul',
        items: [
          'Studio chat, cloud agents, and Test Hub → recorded on each /api/studio/complete/stream turn',
          'Soumtok Desktop agent rounds → recorded on each /api/desktop/agent/round call with the same user_id',
          'Images from Studio or Desktop → POST /api/studio/image (Desktop always uses the API when signed in so usage stays unified)',
          'Voice transcription on Desktop → POST /api/desktop/speech/transcribe when billed to platform keys',
          'Bring-your-own keys (BYOK) → billed_to = user. Still visible in Usage; does not drain included pool',
        ],
      },
      {
        type: 'callout',
      ...
      {
        id: 'meter',
        title: 'How we meter tokens',
        type: 'p',
        text: 'Every LLM call logs prompt_tokens + completion_tokens into usage_events. Trial (hobby) gets 12,000 platform-billed tokens per calendar month on cheap models only. Paid plans convert credits to tokens at 8,000 tokens per credit (Pro = 40 credits ≈ 320K tokens per billing cycle from plan start → renew). When included pool is exhausted, platform routing can continue at on-demand: $2.00 per million total tokens (input + output combined). BYOK bypasses Soumtok keys — you pay the vendor directly at list price.',
      },
      {
        id: 'models',
      ...
      },
      {
        ...
        type: 'p',
        text: 'Still images (Flux 2 Max default) bill as platform tokens, not “1 fake token.” We take Replicate/Fal vendor cost, add 30% platform margin, convert to tokens at the $2/M on-demand rate. Example: Flux 2 Max ≈ $0.06 vendor → ~$0.078 charged → ~39,000 tokens on your Usage row labeled Image. Schnell is cheaper (~1,950 tokens). Images count toward Trial and paid included pools the same as coding turns.',
      },
      {
        id: 'profit',
        title: 'How Soumtok earns',
        type: 'ul',
        items: [
          'Subscriptions — Pro $13.99, Pro Plus $48, Ultra $149, Team seats. Predictable revenue; included tokens are a capacity bundle, not unlimited vendor spend.',
          'Image margin — 30% on top of Replicate/Fal invoice when you use platform keys.',
          'On-demand overflow — $2/M after included pool on platform routing (simple meter; we buy vendor at negotiated/list rates).',
          'No double rent — we do not resell a second VM farm. GitHub stays your code host; we sell the agent loop and model routing.',
        ],
      },
      {
        type: 'callout',
        text: 'We stay cheaper than burning flagship models 24/7 on your own OpenAI bill because most work runs on DeepSeek Flash / mini tiers inside your included pool. You upgrade when you need Opus/GPT-6 daily — not on day one.',
      },
      {
        id: 'compare',
        ...
        type: 'ul',
        items: [
          'Cursor Pro — $20/mo (US). Soumtok Pro — $13.99/mo with M-Pesa/PayPal, same agent + GitHub loop, built in Nairobi.',
      ...
      },
      {
        ...
        items: [
          ...
          'Dashboard → Spending — included % vs BYOK, trial bar, upgrade CTA',
          'Soumtok Desktop → Settings → Usage — live summary from /api/desktop/summary (same database rows)',
          'Each image API response includes tokens and chargeUsd when platform-billed',
        ],
      },
    ],
  },
