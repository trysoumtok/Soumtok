...
function closestWord(token, words) {
...
function repairToken(token, prev, next) {
...
function repairIntentText(text) {
  let out = String(text || '')
    .replace(/\s+/g, ' ')
    .trim()
  ...
  const repairedWords = words.map((word, i) =>
    repairToken(word, (i > 0 ? repairToken(words[i - 1], '', '') : '').toLowerCase(), (words[i + 1] || '').toLowerCase()),
  )
  let wi = 0
  return parts
...
function closestName(want, names) {
